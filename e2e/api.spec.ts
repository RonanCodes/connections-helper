...
test.describe('Connections API', () => {
  ...
  test('GET /api/puzzle/:date returns 404 for invalid date', async ({
    request,
  }) => {
  ...
  test('GET /api/puzzle/:date returns 400 for malformed date', async ({
    request,
  }) => {
  ...
  test('GET /api/definition/:word handles unknown words gracefully', async ({
    request,
  }) => {
    const response = await request.get(
      `${BASE_URL}/api/definition/xyznonexistent123`,
    )
  ...
  test('API responses are cached (second request should be fast)', async ({
    request,
  }) => {
    // First request
