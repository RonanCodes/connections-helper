...
test.describe('Feb 7th 2026 Special Edition - Shapes Puzzle', () => {
  test('API returns image-based puzzle data for 2026-02-07', async ({ request }) => {
  ...
  test('Feb 7th puzzle cards have valid SVG image URLs', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/api/puzzle/2026-02-07`)
    const data = await response.json()
  ...
  test('definitions API handles image words gracefully', async ({ request }) => {
  ...
  test('definitions API handles empty/null words gracefully', async ({ request }) => {
  ...
  test('Feb 7th puzzle loads without errors via full flow', async ({ request }) => {
    // Step 1: Load puzzle
    ...
    const words = puzzle.categories.flatMap((c: any) =>
      c.cards.map((card: any) => card.content || card.image_alt_text).filter(Boolean)
    )
