...
test.describe('Connections Helper', () => {
  ...
  test('shows word definitions after loading', async ({ page }) => {
    ...
    const wordCards = page.locator('.rounded-xl').filter({ hasText: /noun|verb|adjective|Definition/i })
  ...
  test('can toggle answers reveal', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /Hide Answers/i })).toBeVisible()
  ...
  test('theme picker is visible', async ({ page }) => {
    ...
    const themePicker = page.locator('button').filter({ hasText: /🎨|Theme/i }).or(
  ...
  test('persists date in localStorage', async ({ page }) => {
    ...
    // Check localStorage
    const savedDate = await page.evaluate(() => localStorage.getItem('connections-date'))
