...
describe('Integration Tests (Real External APIs)', () => {
  ...
  describe('NYT Puzzle API', () => {
    test('fetches today\'s puzzle from NYT', async () => {
      ...
      const positions = data.categories.flatMap((c: any) => c.cards.map((card: any) => card.position))
  ...
  describe('Urban Dictionary Direct Test', () => {
    test('Urban Dictionary API returns results', async () => {
      ...
      const res = await fetch('https://api.urbandictionary.com/v0/define?term=yeet')
  ...
  describe('End-to-End: Full Puzzle Flow', () => {
    test('fetches puzzle and definitions for all words', async () => {
      ...
      const urbanWords = Object.entries(defs.definitions)
        .filter(([_, def]: [string, any]) => def.source === 'urban')
      ...
      console.log(`   Urban words: ${urbanWords.map(([w]) => w).join(', ') || 'none'}`)
    })
  })
})
