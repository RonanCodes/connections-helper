...
4. **Zod + OpenAPI + Scalar.** Zod schemas for request and response shapes, runtime validation on handlers, auto-generated OpenAPI 3.1 spec served at `/api/openapi.json`, Scalar docs UI at `/api/docs`.
...
- **Docs UI.** Scalar via `@scalar/api-reference`, mounted at `/api/docs` as a TanStack route that returns HTML.
...
- `pnpm dev` serves `/api/openapi.json` and `/api/docs` locally.
