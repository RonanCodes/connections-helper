export const THEMES = ['nyt', 'default', 'light', 'neubrutalism', 'synthwave', 'geocities'] as const;
...
export function getThemeConfig(): ThemeConfig {
  ...
  const theme = (localStorage.getItem('sl-theme') as Theme) || 'default';
  const mode = (localStorage.getItem('sl-mode') as Mode) || 'system';
  return { theme, mode };
}
...
export function cycleTheme(): Theme {
  const current = (localStorage.getItem('sl-theme') as Theme) || 'default';
...
export const THEME_META: Record<Theme, { name: string; description: string; emoji: string }> = {
  nyt: {
    name: 'NYT Games',
